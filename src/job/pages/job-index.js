...
function Job({ navigation, disp_user, appState, disp_surprise, route, disp_Jobs, disp_fetchTime }) {
    ...
    function AvailableJobs() {
        ...
        if (Jobs.length > 0) {
            // if (appState.FetchUserTime.jobFetch == CurrentDate()) {
            //     setLoading(true)
            //     setTimeout(() => {
            //         setLoading(false)
            //         setJobState(Jobs)
            ...
            //                 // }
            //             } else {
            //                 let ActiveJobs = response.data.filter(e => new Date(e.meta.deadline) > new Date())
            //                 console.log(ActiveJobs[1])
