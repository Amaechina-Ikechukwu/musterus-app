...
import {All_Events, surprise_state} from '../../redux';
...
import {GetApp_Campaigns} from '../../donations/controllers/campaign-contrller';
import {DonationCard} from '../../donations/components/campaign-card';
...
function ViewProfile({route, appState, disp_events}) {
  ...
  useEffect(() => {
    ...
    setData(ViewUser);
    FetchUserDetails(ViewUser.phone);
          ...
          <View
            ...
            <View style={{width: '100%'}}>
              <ImageBackground
                ...
                src={
                  data &&
                  `https://ddhqtepvmjgbfndcjrkn.supabase.co/storage/v1/object/public/${data.img}`
                }>
                <View
                  style={[
                    styles.overlay,
                  ...
                  {data && data.img ? (
                    <Pressable
                      ...
                      <Image
                        ...
                        // source={`https://ddhqtepvmjgbfndcjrkn.supabase.co/storage/v1/object/public/${data.img}`}
                        src={`https://ddhqtepvmjgbfndcjrkn.supabase.co/storage/v1/object/public/${data.img}`}
                        resizeMode={'cover'}
                      />
                    </Pressable>
                  ) : (
                    <Avatar.Text
                      size={100}
                      label={`${data && data.name.split(' ')[0][0]} ${
                  ...
                  <View
                    ...
                    <TouchableOpacity
                      ...
                      <Text style={{color: Colors.grey}}>Request Support</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        Alert.alert(
                          'Coming Soon.',
                          'When this feature is rolled out, you will be able to connect with',
                          [{title: 'OK'}],
                        );
                      }}
                      android_ripple={{color: Colors.secondary}}
                      ...
                      <Text style={{color: Colors.grey}}>Connect</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      ...
                      <Text style={{color: Colors.grey}}>Report Account</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              </ImageBackground>
            ...
            <View
              ...
              <View
                ...
                {data && data.meta.zone && (
                  <>
                    <Details
                      data={data && data.meta.zone && data.meta.zone}
                      icon={faEnvelope}
                    />
                  </>
                )}
                    ...
                    {data.study.map((e, key) => {
                      if (e.type == 'highschool') {
                        return (
                          <Certificates
                            key={key}
                            data={`${e.school} (High school class of ${e.meta.class})`}
                            icon={faCheckSquare}
                          />
                        );
                      } else {
                        if (e.isGraduate == true) {
                          return (
                            <Certificates
                              key={key}
                              data={`Studied ${e.course} at ${e.school} ( ${e.meta.fromYear} - ${e.meta.toYear} ) `}
                              icon={faCheckSquare}
                            />
                          );
                        } else {
                          return (
                            <Certificates
                              key={key}
                              data={`Studying ${e.course} at ${e.school} ( since ${e.meta.fromYear}) `}
                              icon={faCheckSquare}
                            />
                          );
                        }
                ...
                {data &&
                  data.meta.certification &&
                  data.meta.certification.length > 0 && (
                    <>
                      <View
                        style={{
                          flexDirection: 'row',
                      ...
                      {data.meta.certification &&
                        data.meta.certification.map((e, key) => {
                          return (
                            <Certificates
                              key={key}
                              data={`${e.certificate} - ${e.year}`}
                              icon={faCheckSquare}
                            />
                          );
                        })}
                ...
                <View style={styles.centeredView}>
                  <Modal
                    ...
                    <Image
                      ...
                      src={`https://ddhqtepvmjgbfndcjrkn.supabase.co/storage/v1/object/public/${
                        data && data.img
                      }`}
                      resizeMode={'cover'}
                    />
        ...
        {reportAccount == true ? (
          <View style={styles.content}>
            <View style={{width: '100%', justifyContent: 'flex-start'}}>
              <Text style={{marginLeft: 20, color: Colors.dark}}>
                Drop a message
              </Text>
              <TextInput
          ...
          <ScrollView>
            <View style={{marginTop: 10, width: '100%'}}>
              <Text style={[Style.Text, {textAlign: 'center'}]}>
                Select the campaign you want {User.name} to support.{' '}
              </Text>
              <Divider style={{marginTop: 20}} />

              {Campaigns.sort(() => 0.5 - Math.random()).map((e, index) => {
                return (
                  <DonationCard
                    data={e}
